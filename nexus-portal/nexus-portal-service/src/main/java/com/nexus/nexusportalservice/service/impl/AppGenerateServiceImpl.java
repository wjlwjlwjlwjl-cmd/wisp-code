...
public class AppGenerateServiceImpl implements IAppGenerateService {
    ...
    private String getSystemPrompt(String appId) {
        return String.join("\n",
                ...
                "- **index.html 强制模板（关键，务必逐字遵守，否则构建/挂载会失败）**:",
                " - `index.html` 只能是⼀个「极简壳」：`<head>` 内仅放 `charset`/`viewport`/`title`；`<body>` 内**只允许** `<div id=\"app\"></div>` 与⼀个外链脚本 `<script type=\"module\" src=\"/src/main.js\"></script>`。",
                " - **严禁**在 `index.html` 中出现：内联 `<script type=\"module\">`、`import` 任何 `.vue`/`.js`、`createApp(...)`、或直接书写 `@click`/`v-if`/`{{ }}` 等 Vue 指令与插值。Vite 会把内联 module 当作 `index.html?html-proxy` 以项⽬根为基准解析，导致 `Could not resolve \"./App.vue\"` 构建失败。",
                " - 所有⻚⾯结构/样式/交互逻辑⼀律写⼊ `src/App.vue`（及 `src/**/*.vue`、`src/**/*.css`）；`src/main.js` 只负责 `createApp(App).mount('#app')`（如使⽤路由则装配 router）。",
                "- **配置强制要求**:",
